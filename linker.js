...
var Linker = function (ParentClass, ChildClass, relationName, values) {
  ...
  values.forEach(function(value) {
    ...
    // when all the queries have completed, nest the children in the parent object and save the happy batch
    var promise = Q.all(queries)
      .then(function(results) {
        ...
        return parent.save(null, {useMasterKey: true});
      });
  ...
  return Q.all(allThePromisesWeMade)
    .then(function() {
      console.log(ParentClass.prototype.className + '.' + ChildClass.prototype.className + ': done.');
    }, errorHandler);
};
